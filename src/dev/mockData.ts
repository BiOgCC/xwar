...
export function initMockData() {
  ...
  if (bountyState.bounties.length === 0) {
    ...
    useBountyStore.setState({
      bounties: [
        { id: 'bounty_1', targetName: 'Rogue_Agent', placedBy: playerName, reward: 50000, reason: 'War criminal', status: 'active' as const, createdAt: now - 86400000, claimedBy: null },
  ...
  // ═══════════════════════════════════════════
  //  11. MILITARY UNIT — DISABLED: MU is now player-created only.
  //      No NPC members — real players join through the game UI.
  // ═══════════════════════════════════════════
}
