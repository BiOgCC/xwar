  ...
  return (
    <>
      {isOfficial && (
        <div className="hud-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '6px' }}>
            ...
            {lawType === 'authorize_nuclear_action' && <p style={{ fontSize: '9px', color: '#f59e0b', margin: 0 }}>⚠️ Requires national fund to meet nuke resource requirements.</p>}
      ...
      <div className="hud-card">
        ...
        {activeLaws.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b', marginTop: '6px' }}>No laws on the floor.</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '6px' }}>
            {activeLaws.map(law => (
              <div key={law.id} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)', padding: '8px', borderRadius: '3px' }}>
                <div style={{ fontSize: '10px', color: '#38bdf8', fontWeight: 700, textTransform: 'uppercase' }}>{law.type.replace(/_/g, ' ')}</div>
