...
export const useCyberStore = create<CyberState>((set, get) => ({
  ...
  launchCampaign: (operationType, target, invitedPlayers) => {
    ...
    import('../api/client').then(({ cyberLaunch }) => cyberLaunch(operationType, target.country, target.region, target.player).catch(() => {}))
  ...
  contributeToCampaign: (campaignId, amount, type) => {
    ...
    // Persist to backend (fire-and-forget)
    import('../api/client').then(({ cyberContribute }) => cyberContribute(campaign.operationType).catch(() => {}))
    return true
  },
